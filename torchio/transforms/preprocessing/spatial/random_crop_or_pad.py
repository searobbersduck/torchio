...
class RandomCropOrPad(BoundsTransform):
    ...
    @staticmethod
    def _get_six_bounds_parameters_random(
            parameters: np.ndarray,
            ) -> TypeSixBounds:
        r"""Compute bounds parameters for ITK filters.
        ...
        """  # noqa: E501
        # parameters = parameters / 2
        parameters_lower = np.array([np.random.randint(0,i+1) for i in parameters])
        result = []
        for number_lower, number in zip(parameters_lower, parameters):
            ini, fin = int(np.ceil(number_lower)), int(np.floor(number-number_lower))
            result.extend([ini, fin])
        return tuple(result)
    ...
    def _compute_cropping_padding_from_shapes(
        ...
        if cropping.any():
            cropping_params = self._get_six_bounds_parameters_random(cropping)
        else:
            cropping_params = None

        padding = np.maximum(diff_shape, 0)
        if padding.any():
            padding_params = self._get_six_bounds_parameters_random(padding)
        else:
            padding_params = None
